# Hosts Compress - Windows

![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=Lateralus138&repo=hosts-compress-windows)

---

***This project is*** ![Under Construction](https://img.shields.io/static/v1?logo=data:image/png;base64,&label=Under&message=Construction&style=for-the-badge&labelColor=1D1D1D&color=ffff99), but coming very soon.

For now there are (much slower than this, but working well) scripts for Windows and Linux at my other *hosts compression* project located [***here***](https://github.com/Lateralus138/hosts-compression-scripts).
